...
def choose_seat_view(request):
    ...
    popcorn=request.POST.get('Popcorn')
...
def proceed_to_pay_view(request):
    total1=int(request.COOKIES['allNumberVals'])*100+int(p1)*60+int(p2)*40
    #popcorn=int(request.COOKIES['Popcorn'])*60
    #pepsi=int(request.COOKIES['Pepsi'])*40
    #total=total1+popcorn+pepsi
    totalSeat=int(request.COOKIES['allNumberVals'])
    return render(request,'index.html',{'total':total1,'totalSeat':totalSeat})
...
def payment_success_view(request):
    ...
    total=int(request.COOKIES['allNumberVals'])*100+int(p1)*60+int(p2)*40
